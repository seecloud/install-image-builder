...
function mkprep()
{
  ...
  yum_update_cache /etc/yum.conf 1
...
function main() {
  ...
  cp   /etc/pki/rpm-gpg/RPM-GPG-KEY-CentOS-SIG-Storage ${builddir}/RPM-GPG-KEY-CentOS-SIG-Storage
  # gen yum.conf files
...
EOF
  ...
  yum_update_cache ${tmp_yumroot}/yum.conf 1
  # downloading updated packages
  repotrack -c ${tmp_yumroot}/yum.conf -a x86_64 -r base -r extras -r epel -r updates -r centos-gluster38  -p ${tmp_downloaded_packages} ${RPM_PACKAGES} || exit $?
...
cp /run/install/repo/RPM-GPG-KEY-CentOS-SIG-Storage /mnt/sysimage/etc/pki/rpm-gpg/RPM-GPG-KEY-CentOS-SIG-Storage
